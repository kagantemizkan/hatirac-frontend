...
const ComplatedUpload = (props) => {
  ...
  const qrCodeUrl = `https://chart.googleapis.com/chart?chs=250x250&cht=qr&chl=https://hatirac.com/show/${props.data.userid}`
  ...
  return (
    <div className='container d-flex flex-column justify-content-center align-items-center text-center mt-4'>
      ...
      <h5 className='text-muted'>Kilidin Açılacağı Tarih: <b className='text-bold text-darkBlue'>{formatDate(props.data.lock_date)}</b></h5>
      <h5 className='text-muted'>Mesajınız: <b className='text-bold text-darkBlue'>{props.data.message}</b></h5>
      <h5 className='text-muted'>Link: <b className='text-bold text-darkBlue'><a href={`https://hatirac.com/show/${props.data.userid}`} target='_blank' rel="noreferrer">Buraya tıklayarak ürününüzü göndereceğiniz kişinin göreceği ekranı takip edebilirsiniz.</a></b></h5>
